...
export const pickupNames:Record<PickupKind,string>={juice:'Juice box',stuffie:'Teddy shield',toy:'Squeaky toy',bandage:'Bandage kit'};
export class Pickups {
  ...
  reset(){this.items.forEach(i=>this.remove(i));this.items=[];this.timer=0;this.next=0;this.spawn(0,'juice');this.spawn(1,'stuffie');this.spawn(2,'bandage');}
  ...
  spawn(slot:number,kind:PickupKind){const g=new THREE.Group(),p=this.slots[slot];g.position.set(p.x,.28,p.z);this.scene.add(g);
    ...
    else if(kind==='bandage'){box(g,.28,.22,.14,0,0,0,'#f3eddb');box(g,.15,.045,.015,0,0,.079,'#bd7057');box(g,.045,.15,.015,0,0,.081,'#bd7057');}
  ...
  update(dt:number,time:number,player:V2,dog:V2){this.timer+=dt;for(const i of this.items){i.mesh.position.y=.32+Math.sin(time*2.4+i.position.x)*.055;i.mesh.rotation.y=time*.65;}
    if(this.timer>=25){this.timer=0;if(this.items.length>=3)return;const slot=this.slots.findIndex(p=>!this.items.some(i=>i.position===p)&&distance(player,p)>1.5&&distance(dog,p)>1.5);if(slot>=0){const kinds:PickupKind[]=['bandage','toy','juice','stuffie'];this.spawn(slot,kinds[this.next++%kinds.length]);}}
  }
