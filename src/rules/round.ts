...
export type PickupKind='juice'|'stuffie'|'toy'|'bandage';
export interface RoundState {
  time:number; hp:number; anger:number; catches:number; treats:number; progress:number; score:number;
  chain:number; bestChain:number; lastCatch:number; clean:boolean; lastHit:number; hitCooldown:number; stagger:number; knockedDown:number; softSlow:number;
...
export function catchTail(s:RoundState):number {
  if(s.ended||s.warning!==null||s.immunity>0||s.knockedDown>0)return 0;
...
export function hardImpact(s:RoundState,speed:number){if(s.ended||s.hitCooldown>0||s.knockedDown>0)return 0;let damage=impactDamage(speed);if(!damage)return 0;if(s.buff==='stuffie')damage=Math.ceil(damage/2);s.hp=Math.max(0,s.hp-damage);s.hits++;s.chain=0;s.clean=false;s.lastHit=s.time;s.hitCooldown=1;s.stagger=speed<3.2?.25:speed<4.5?B.stumbleTime:0;s.knockedDown=speed>=4.5?B.fallTime:0;if(s.hp<=0)finish(s,'health');return damage;}
export function spendTreat(s:RoundState){if(s.ended||s.knockedDown>0||s.treats<1||s.treatCooldown>0)return false;s.treats--;s.treatsUsed++;s.treatCooldown=B.treatCooldown;return true;}
...
export function tickRules(s:RoundState,dt:number,canCool:boolean,eating:boolean){
  ...
  for(const key of ['hitCooldown','stagger','knockedDown','softSlow','immunity','grabCooldown','treatCooldown','buffTime'] as const)s[key]=Math.max(0,s[key]-dt);
...
export function healWithPickup(s:RoundState){if(s.ended||s.knockedDown>0||s.pickup!=='bandage'||s.hp>=100)return 0;const healed=Math.min(B.healAmount,100-s.hp);s.hp+=healed;s.pickup=null;return healed;}
