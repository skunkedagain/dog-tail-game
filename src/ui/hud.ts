...
export class UI {
  ...
  toastTime=0;last=0;touchMode=false;
  constructor(app:HTMLElement){app.innerHTML=`
    <canvas id="game" aria-label="First-person Dog Tail Game. WASD to move, arrow keys or mouse to look, Space to catch, C to jump." tabindex="-1"></canvas>
    ...
    <div class="hud" id="hud" hidden>
      ...
      <div class="play-help" id="play-help">WASD move · Arrows / mouse look · Space / click grab · C jump</div>
    </div>
    <div class="reticle" id="reticle" hidden><div class="crosshair"></div><span id="reach-label"></span></div>
    <div id="touch-controls" class="touch-controls" aria-label="Touch game controls" hidden>
      <div id="move-stick" class="move-stick" role="group" aria-label="Movement joystick. Push to the edge to sprint."><div id="move-thumb"></div><span>MOVE · EDGE TO SPRINT</span></div>
      <div class="touch-actions">
        <button type="button" data-action="KeyE" aria-label="Offer treat">Treat</button>
        <button type="button" data-action="KeyQ" aria-label="Use pickup">Use</button>
        <button type="button" data-action="KeyF" aria-label="Swap pickup">Swap</button>
        <button type="button" data-action="KeyC" class="touch-jump" aria-label="Jump">↑<small>JUMP</small></button>
        <button type="button" data-action="Grab" class="touch-catch" aria-label="Catch tail">✦<small>CATCH</small></button>
      </div>
      <span class="touch-look-hint">TAP TO CATCH · SWIPE TO LOOK</span>
    </div>
  ...
  title(best:number){this.menu.classList.remove('results');this.menu.innerHTML=`<div class="eyebrow">LITTLE FEET. BIG MISCHIEF.</div><h1>Catch the tail.<br><em>Cause a little chaos.</em></h1><p>Three minutes. One very good dog.<br>And a toddler with absolutely no chill.</p><button id="start" class="primary">Let's play <span>↗</span></button><div class="menu-meta"><span>${best?`PERSONAL BEST · ${best.toLocaleString()}`:'ONE ROOM · THREE MINUTES'}</span><button id="settings" class="text-button">Settings & controls</button><button id="high-scores" class="text-button">High scores</button></div><div class="how keyboard-help"><span><kbd>W A S D</kbd> Move <kbd>SHIFT</kbd> Sprint <kbd>C</kbd> Jump</span><span><kbd>↑ ↓ ← →</kbd> / Mouse · Look</span><span><kbd>SPACE</kbd> / Click · Catch the tail</span><span><kbd>E</kbd> Treat <kbd>Q</kbd> Use pickup</span></div><div class="touch-instructions"><span>Left thumb moves. Swipe the room to look.</span><span>Tap the room to catch the tail. Tap Jump to hop.</span><span>Push the stick to its edge to sprint.</span></div><p class="device-note">${this.touchMode?'Ready for little thumbs. Landscape gives you more room.':'Made for a keyboard. Arrow keys work without mouse lock.'}</p>`;this.showMenu();}
  setTouchMode(enabled:boolean){this.touchMode=enabled;document.body.classList.toggle('touch-mode',enabled);this.el('touch-controls').hidden=!enabled||!document.body.classList.contains('playing');}
  showMenu(){this.el('touch-controls').hidden=true;this.el('menu-layer').hidden=false;this.menu.hidden=false;this.hud.hidden=true;this.reticle.hidden=true;this.warning.hidden=true;this.toast.hidden=true;this.toastTime=0;this.el('corner-note').hidden=false;this.el('pause-button').hidden=true;document.body.classList.remove('playing');}
  play(){this.el('touch-controls').hidden=!this.touchMode;this.el('menu-layer').hidden=true;this.menu.hidden=true;this.hud.hidden=false;this.reticle.hidden=false;this.el('corner-note').hidden=true;this.el('pause-button').hidden=false;document.body.classList.add('playing');}
  ...
  settings(s:Settings){this.menu.classList.remove('results');this.menu.innerHTML=`<div class="eyebrow">MAKE YOURSELF AT HOME</div><h1 class="smaller-title">Your kind<br>of <em>chaos.</em></h1><div class="settings-grid"><label>Look sensitivity <input id="sensitivity" type="range" min="0.3" max="2" step="0.1" value="${s.sensitivity}" /></label><label>Field of view <input id="fov" type="range" min="60" max="90" value="${s.fov}" /></label>${([['bob','Gentle camera bob'],['sound','Sound effects'],['invert','Invert mouse Y'],['shadows','Detailed shadows'],['touch','Touch controls']] as const).map(([id,label])=>`<label class="check-row">${label}<input type="checkbox" id="${id}" ${s[id]?'checked':''}/></label>`).join('')}</div><p class="settings-help">WASD move · Shift sprint<br>Arrow keys / mouse look · Space / click grab · C jump<br>E treat · Q use pickup · F swap pickup<br>Touch: tap the room to grab; swipe to look.<br>Left stick moves (edge sprints).<br>Jump, Catch, Treat, Use and Swap have separate buttons.<br>Right-drag to look when mouse lock is unavailable.<br>Escape pauses. Earn a treat every 3 catches.<br>Hard sprint impacts knock you down. Bandage kits restore HP.</p><button class="primary" id="back">All set <span>✓</span></button>`;this.showMenu();this.el('back').focus();}
  ...
  update(s:RoundState,dt:number,ready:boolean,reachLabel:string,locked:boolean){
    ...
    this.el('pickup').textContent=s.pickup?pickupNames[s.pickup]:'Empty pocket';this.el('buff').textContent=s.buff?`${pickupNames[s.buff]} · ${Math.ceil(s.buffTime)}s`:s.pickup==='bandage'?`${this.touchMode?'Use':'Q'} · Restore up to 30 HP`:s.pickup?(this.touchMode?'Tap Use':'Press Q to use'):'Find a little advantage.';
    this.el('touch-controls').querySelector('.touch-catch')!.classList.toggle('ready',ready);
    this.reticle.classList.toggle('ready',ready);this.el('reach-label').textContent=reachLabel;this.el('play-help').textContent=this.touchMode?'Left stick move · Swipe look · Tap the room to catch':locked?'WASD move · Shift sprint · Arrows / mouse look · Space / click grab · C jump':'WASD move · Arrows look · Space grab · C jump · Right-drag mouse look';
    this.warning.hidden=s.warning===null;if(s.warning!==null)this.warning.textContent=`ENOUGH!  ${s.warning.toFixed(1)}s · ${s.treats?(this.touchMode?'Tap Treat — quick!':'Press E — offer a treat!'):'No treats left. Time-out is coming!'}`;
  }
}
