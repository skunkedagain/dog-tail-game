...
export class UI {
  ...
  update(s:RoundState,dt:number,ready:boolean,reachLabel:string,locked:boolean){
    ...
    this.el('pickup').textContent=s.pickup?pickupNames[s.pickup]:'Empty pocket';this.el('buff').textContent=s.buff?`${pickupNames[s.buff]} · ${Math.ceil(s.buffTime)}s`:s.pickup?'Press Q to use':'Find a little advantage.';
