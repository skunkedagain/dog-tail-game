...
export type Settings={sensitivity:number;fov:number;bob:boolean;sound:boolean;invert:boolean;shadows:boolean;touch:boolean};
export class Controls {
  ...
  constructor(readonly canvas:HTMLCanvasElement,readonly settings:Settings){
    ...
    canvas.addEventListener('pointerdown',e=>{if(!this.active||e.pointerType!=='mouse')return;if(e.button===0)this.edges.add('Grab');if(e.button===2)this.dragging=true;});
  ...
  async lock(){if(this.settings.touch)return;try{await this.canvas.requestPointerLock();}catch{/* Arrow keys and right-drag remain available. */}}
  unlock(){if(this.locked)document.exitPointerLock();}
  reset(){this.keys.clear();this.edges.clear();this.dragging=false;this.touch.reset();}
