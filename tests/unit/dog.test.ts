...
it('navigates a three-minute chase with capped progression without getting stuck',async()=>{
  ...
  for(let i=0;i<10800;i++){
    ...
    ai.update(1/60,actor.position,player,s,actor.grounded);if(ai.jumpRequested)physics.jump(actor);physics.move(actor,ai.velocity,1/60);physics.step();total+=distance(old,actor.position);
